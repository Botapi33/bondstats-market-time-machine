...
function draw(){drawChart($('#market-chart'),state.moment.series,state.visibleIndex,state.baseIndex);const max=Math.max(...state.moment.series.map(s=>s.values.length))-1;const pct=Math.max(0,100-(state.visibleIndex/max*100));$('#future-mask').style.width=`${Math.max(0,Math.min(70,pct))}%`;$('#future-mask').hidden=state.visibleIndex>=max}
