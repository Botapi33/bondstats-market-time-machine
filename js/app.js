...
async function boot(){
  ...
  const [moments,sources]=await Promise.all([getJSON('./data/moments.json'),getJSON('./data/sources.json')]);
  if(!Array.isArray(moments)||!moments.length) throw new Error('moments.json is empty or invalid');
...
function selectMoment(id,push){
  ...
  $('#chart-title').textContent=m.chartTitle||'Treasury yields';$('#chart-asof').textContent=`As of ${prettyDate(m.date)}`;
...
function renderHorizons(){
  const m=state.moment;const horizons=[['1D',1],['7D',7],['30D',30],['90D',90],['1Y',365]];
  $('#horizon-buttons').innerHTML=horizons.map(([label,days])=>{
    const h=m.outcomes?.find(o=>o.days===days);
    return `<button class="horizon-button" data-days="${days}" ${h?'':'disabled'} title="${h?prettyDate(h.date):'Historical observation unavailable'}">${label}</button>`
  }).join('');
  document.querySelectorAll('.horizon-button:not(:disabled)').forEach(b=>b.addEventListener('click',()=>advance(Number(b.dataset.days))));
  const coverage=document.querySelector('#horizon-coverage');
  if(coverage) coverage.textContent=`Daily market path · ${m.series?.[0]?.values?.length||0} aligned observations · 1D / 7D / 30D / 90D / 1Y`;
}
function advance(days){const o=state.moment.outcomes.find(x=>x.days===days);if(!o)return;setVisibleIndex(o.visibleIndex);document.querySelectorAll('.horizon-button').forEach(b=>b.classList.toggle('is-selected',Number(b.dataset.days)===days));$('#chart-asof').textContent=`Information available through ${prettyDate(o.date)}`;$('#outcome-title').textContent=`${days===365?'1 year':days+' days'} later · ${prettyDate(o.date)}`;const base=state.moment.snapshot;const cards=[['U.S. 2Y',Number.isFinite(o.us2y)&&Number.isFinite(base.us2y)?(o.us2y-base.us2y)*100:null,fmtBp],['U.S. 10Y',Number.isFinite(o.us10y)&&Number.isFinite(base.us10y)?(o.us10y-base.us10y)*100:null,fmtBp],['2s10s',Number.isFinite(o.us2y)&&Number.isFinite(o.us10y)&&Number.isFinite(base.us2y)&&Number.isFinite(base.us10y)?((o.us10y-o.us2y)-(base.us10y-base.us2y))*100:null,fmtBp],['VIX',Number.isFinite(o.vix)&&Number.isFinite(base.volatility)?o.vix-base.volatility:null,v=>Number.isFinite(v)?`${v>0?'+':''}${v.toFixed(1)}`:'—']];$('#outcome-grid').innerHTML=cards.map(([l,v,f])=>`<div class="outcome-card"><span>${l}</span><strong>${f(v)}</strong></div>`).join('');$('#outcome-panel').hidden=false;$('#outcome-panel').scrollIntoView({behavior:'smooth',block:'nearest'})}
function setVisibleIndex(i){state.visibleIndex=i;$('#outcome-panel').hidden=i===state.baseIndex;if(i===state.baseIndex){$('#chart-asof').textContent=`As of ${prettyDate(state.moment.date)}`;document.querySelectorAll('.horizon-button').forEach(b=>b.classList.remove('is-selected'));}draw()}
function draw(){drawChart($('#market-chart'),state.moment.series,state.visibleIndex,state.baseIndex)}
...
boot().catch(err=>{
  ...
  const title=$('#moment-title'); if(title) title.textContent='Historical data unavailable';
  const kicker=$('#moment-kicker'); if(kicker) kicker.textContent='The interface loaded, but the historical dataset could not be read. Reload the page or verify that data/moments.json is present.';
  const list=$('#moment-list'); if(list) list.innerHTML='<div style="padding:14px;color:#8e8b84;font-size:12px;line-height:1.5">No market moments loaded.</div>';
});
